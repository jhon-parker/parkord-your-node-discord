...
import { Settings, Trash2, Lock, Unlock, UserX, Ban, Edit2, Plus, X } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const ServerSettings = ({
  ...
  const handleSaveServer = async () => {
    ...
    const { error } = await supabase
      .from("servers")
      .update({ name: serverName, is_private: isPrivate })
      .eq("id", server.id);
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Только участники смогут видеть сервер
                </p>
              </div>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent className="bg-card border-border">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Это действие нельзя отменить. Все каналы и сообщения будут удалены.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                  ...
                  {bannedUsers.map((ban) => (
                    ...
                    >
                      <span className="text-foreground">
                        {ban.profiles?.username || "Пользователь"}
                      </span>
                      <Button
                        size="sm"
                        variant="outline"
